	...
	<div
		...
		<div class="flex items-center justify-between w-full mb-6">
			<div class="relative w-full ml-4">
				<BaseInput type="text" name="search" placeholder="البحث عن فعالية" v-model="search" :icon="true" label=""> </BaseInput>
		...
		<div class="grid w-full grid-cols-1 gap-4">
			<div
				...
				:key="item.id"
				class="py-5 px-6 bg-white rounded-lg border border-[#EAECF0] flex flex-col md:flex-row items-start md:items-center justify-between">
				<div class="flex items-center">
					<div class="px-2 py-4 ml-4 text-center rounded-lg" :class="[item.status]">
					...
					<div class="flex flex-col items-start justify-center space-y-3">
						<p class="text-[#3F3F3F] text-[24px] leading-[18px] font-bold">{{ item.title }}</p>
						<p class="text-[#3F3F3F] text-[16px] leading-[18px]">
							{{ item.category
							}}<span class="text-[#158456]" :class="[item.status === 'upcoming' ? 'text-[#158456]' : 'text-[#912018]']">{{
				...
				<button v-if="item.status === 'upcoming'" class="text-[16px] text-[#307094] hover:underline mt-3 md:mt-0">
					سجل الآن
				</button>
				<p v-else class="text-[16px] text-[#FF3A46] mt-3 md:mt-0">التسجيل مغلق</p>
			</div>
		</div>
